...
export class DatabaseInitService implements OnModuleInit {
  ...
  private async migrateRoles() {
    ...
    `);
    await this.ds.query(
      `UPDATE users SET role = 'employee' WHERE role IS NULL OR role NOT IN ('admin', 'employee', 'bu', 'manager', 'hr')`,
    );
    ...
    await this.ds.query(
      `ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('admin', 'employee', 'bu', 'manager', 'hr'))`,
    );
    this.logger.log('RBAC roles migrated to admin | employee | bu | manager | hr');
  }
  ...
  private async migrateRegularizationRequestType() {
    ...
    await this.ds.query(
      `UPDATE wfh_regularization_requests SET request_type = 'wfh' WHERE request_type IS NULL OR request_type NOT IN ('wfh', 'mark_present', 'late_in', 'early_out')`,
    );
    await this.ds.query(
    ...
    try {
      await this.ds.query(
        `ALTER TABLE wfh_regularization_requests ADD CONSTRAINT wfh_requests_type_check CHECK (request_type IN ('wfh', 'mark_present', 'late_in', 'early_out'))`,
      );
    } catch (err) {
      const message = (err as Error).message || '';
      if (!/already exists/i.test(message)) throw err;
